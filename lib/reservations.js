...
import { generateRUR } from '@/lib/rurGenerator';
...
export async function saveReservation(formData, planDetails) {
  ...
  try {
    // 0. Generar RUR antes de insertar
    const rur = await generateRUR(selectedActivity);
...
async function saveDynamicPlanDetails(reservaId, details) {
  ...
  // 2. Guardar solo las opciones dinámicas que existen en BD
  // Filtrar: solo opciones con IDs que existen en opciones_dinamicas
  const opcionesValidasIds = ['transport', 'breakfast', 'snack', 'lunch', 'photography', 'biology', 'bilingual'];
  
  if (opcionesSeleccionadas && opcionesSeleccionadas.length > 0) {
    const opcionesData = opcionesSeleccionadas
      .filter(opcion => opcionesValidasIds.includes(opcion.id))
      .map(opcion => ({

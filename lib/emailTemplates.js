...
export function generateClientEmail(data) {
  const { nombre, apellido, destino, sendero, plan, fecha, visitantes, total, planDetails, descripcion, rur } = data;
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
          ...
          <tr>
            <td style="background: linear-gradient(135deg, #1f2937 0%, #374151 100%); padding: 40px 30px; text-align: center;">
              ...
              ${rur ? `<p style="color: #EAB308; margin: 10px 0 0 0; font-size: 14px; font-weight: bold; letter-spacing: 1px;">${rur}</p>` : ''}
            </td>
          </tr>
                ...
                <tr>
                  <td style="padding: 20px;">
                    ...
                    <table width="100%" cellpadding="8" cellspacing="0">
                      ${rur ? `
                      <tr style="background-color: #fef3c7; border: 2px solid #EAB308;">
                        <td style="color: #92400e; font-size: 14px; font-weight: bold; width: 40%;">Código de Reserva:</td>
                        <td style="color: #92400e; font-size: 16px; font-weight: bold; letter-spacing: 1px;">${rur}</td>
                      </tr>
                      ` : ''}
                      <tr>
                    ...
                    ${rur ? `
                    <div style="margin-top: 15px; padding: 12px; background-color: #dbeafe; border-left: 4px solid #3b82f6; border-radius: 4px;">
                      <p style="color: #1e40af; margin: 0; font-size: 13px; line-height: 1.6;">
                        <strong>⚠️ Importante:</strong> Guarda tu código de reserva <strong>${rur}</strong>. Lo necesitarás para cualquier consulta o modificación.
                      </p>
                    </div>
                    ` : ''}
                  </td>
...
export function generateAdminEmail(data) {
  ...
  const atenderUrl = `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/atender-reserva`;
  ...
  // Agregar botón de acción antes del footer
  const buttonHTML = `
    <!-- Botón de Acción Admin -->
    <tr>
      <td style="padding: 0 30px 30px 30px;">
        <table width="100%" cellpadding="0" cellspacing="0" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); border-radius: 8px;">
          <tr>
            <td style="padding: 20px; text-align: center;">
              ...
              <p style="color: #ffffff; margin: 0 0 20px 0; font-size: 14px;">
                Haz clic en el botón para atender esta reserva y enviar el link de registro al cliente.
              </p>
              <a href="${atenderUrl}?rur=${encodeURIComponent(rur)}&amp;token=${token}" 
                 style="display: inline-block; background-color: #ffffff; color: #059669; padding: 15px 40px; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px;">
                ✅ Atender Reserva
              </a>
              <p style="color: #d1fae5; margin: 15px 0 0 0; font-size: 12px;">
                Esto cambiará el estado a "atendida" y enviará el email de registro al cliente.
              </p>
            </td>
          </tr>
        </table>
...
// EMAIL PASO 2: Registro de acompañantes
export function generateRegistrationEmail(data) {
  const { nombre, apellido, rur, linkRegistro, total, fecha } = data;
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
          ...
          <tr>
            <td style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); padding: 40px 30px; text-align: center;">
              <h1 style="color: #ffffff; margin: 0; font-size: 28px;">✅ Reserva Atendida</h1>
              <p style="color: #ffffff; margin: 10px 0 0 0; font-size: 16px;">ALMONTE by Cladonia S.A.S</p>
              <p style="color: #d1fae5; margin: 10px 0 0 0; font-size: 14px; font-weight: bold;">${rur}</p>
            </td>
          </tr>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="background: linear-gradient(135deg, #EAB308 0%, #F59E0B 100%); border-radius: 8px;">
                <tr>
                  <td style="padding: 30px; text-align: center;">
                    <h3 style="color: #ffffff; margin: 0 0 15px 0; font-size: 20px;">📝 Completa tu Registro</h3>
                    <p style="color: #ffffff; margin: 0 0 25px 0; font-size: 14px;">
                      Haz clic en el botón para registrar los datos de todos los acompañantes
                    </p>
                    <a href="${linkRegistro}" 
                       style="display: inline-block; background-color: #ffffff; color: #F59E0B; padding: 15px 40px; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px;">
...
// EMAIL PASO 3: Factura y confirmación final
export function generateInvoiceEmail(data) {
  const { nombre, apellido, rur, total, montoPagado, metodoPago, referenciaPago, fecha } = data;
...
// EMAIL ADMIN: Confirmación de visitantes registrados
...
// Helper para generar token de confirmación de pago
